...
class mineLabel(QtWidgets.QLabel):
    ...
    def mousePressEvent(self, e):  # 重载一下鼠标点击事件
        ...
        if e.buttons() == QtCore.Qt.LeftButton | QtCore.Qt.RightButton:
            self.leftAndRightPressed.emit(yy // self.pixSize,
                                          xx // self.pixSize)
            self.leftAndRightClicked = True
        else:
            if e.buttons() == QtCore.Qt.LeftButton:
    ...
    def mouseReleaseEvent(self, e):
        ...
        if self.leftAndRightClicked:
            self.leftAndRightRelease.emit(yy // self.pixSize,
                                          xx // self.pixSize)
            self.leftAndRightClicked = False
        else:
    ...
    def mouseMoveEvent(self, e):
        ...
        if self.game.timeStart == True and self.game.finish == False:
            if self.lastcell != None:
                self.game.path += (
                    ((yy - self.lastcell[1])**2 +
                     (xx - self.lastcell[0])**2)**0.5) / self.pixSize
            self.lastcell = (xx, yy)
    ...
    def paintEvent(self, event):
        ...
        for i in range(self.row):
            for j in range(self.column):
                painter.drawPixmap(j * size, i * size,
                                   self.pixmaps[self.getPixmapIndex(i, j)])
        painter.end()
