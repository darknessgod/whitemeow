...
class Counter(object):
    ...
    def __init__(self, counterWindow, game):
        ...
        self.lineheight = 18
        self.rowsnames = [
            'RTime', 'Est time', '3BV', '3BV/s', 'QG', 'RQP', 'Ops', 'Isls',
            'LRD', 'Flags', 'Cl', 'Ce', 'Path', 'IOE', 'Corr', 'ThrP', 'Games',
            'Ranks'
        ]
    ...
    def refreshvalues(self, status):
        ...
        if status == 2:
            ...
            if solvedbv == 0:
                ...
                est = rt / (solvedbv + 2 * self.game.solvedops) * (
                    allbv + 2 * self.game.ops)
            values = [
                '%.2f' % (rt),
                '%.2f' % (est),
                '%d/%d' % (solvedbv, allbv),
                '%.3f' % (solvedbv / rt),
                '%.3f' % (pow(est, 1.7) / allbv)
            ]
            values += [
                '%.2f' % (est * (est + 1) / allbv),
                '%d/%d' % (self.game.solvedops, self.game.ops),
                '%d/%d' % (self.game.solvedislands, self.game.islands)
            ]
            values += [
                '%d/%d/%d' % (self.game.allclicks[0], self.game.allclicks[1],
                              self.game.allclicks[2])
            ]
            ...
            values += [
                '%d' % (self.game.gamenum),
                '%d/%d/%d' %
                (self.game.ranks[0], self.game.ranks[1], self.game.ranks[2])
            ]
