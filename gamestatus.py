...
class gamestatus(object):
    ...
    def __init__(self, row, column, mines, options):
        ...
        self.num = [[0 for j in range(self.column)]
                    for i in range(self.row)]  # -1雷，0-8数字
        self.status = [[0 for j in range(self.column)]
                       for i in range(self.row)]  # 0未开 1打开 2标雷
        self.pressed = [[0 for j in range(self.column)]
                        for i in range(self.row)]  # 0未打开 1打开 2标雷 3以上是其他
            ...
            for i in range(num):
                ...
                for i in range(r - 1, r + 2):
                    for j in range(c - 1, c + 2):
                        if not self.outOfBorder(i,
                                                j) and (self.num[i][j] != -1):
                            self.num[i][j] += 1
            ...
            if self.num[i][j] == 0:  #左键开op递归
                for r in range(i - 1, i + 2):
                    for c in range(j - 1, j + 2):
                        if not self.outOfBorder(r, c) and self.status[r][
                                c] == 0 and self.num[r][c] != -1:
                            self.status[r][c] = 1
                ...
                for r in range(i - 1, i + 2):
                    for c in range(j - 1, j + 2):
                        if not self.outOfBorder(r,
                                                c) and self.status[r][c] == 2:
                            flagged += 1
                    ...
                    for r in range(i - 1, i + 2):
                        for c in range(j - 1, j + 2):
                            if not self.outOfBorder(r, c) and self.status[r][
                                ...
                                self.status[r][c] = 1
                                self.num0queue.put([r, c, start0])
                            elif not self.outOfBorder(r, c) and self.num[r][
                                    c] == -1 and self.status[r][c] == 0:
                                self.failed = True
                                self.redmine = [r, c]
    ...
    def domove(self, i, j):
        if not self.outOfBorder(i, j):
            if (i, j) != self.oldCell and (self.leftAndRightHeld
                                           or self.leftHeld):
                ii, jj = self.oldCell
                self.oldCell = (i, j)
        ...
        for i in range(self.row):
            for j in range(self.column):
                if result == 2:  #输了
                    ...
                    if self.num[i][j] == -1 or self.status[i][j] == 2:
                        if self.num[i][j] == -1 and self.status[i][
                                j] == 0:  #游戏过程中未标上的雷
                            self.pressed[i][j] = 5
                        elif self.num[i][j] == -1 and self.status[i][
                                j] == 2:  #游戏过程中标上的雷
                            pass
                        else:
                            self.pressed[i][j] = 3
    ...
    def chordingFlag(self, i, j):
        # i, j 周围标雷数是否满足双击的要求
        if self.num[i][j] <= 8 and self.num[i][j] >= 0 and self.status[i][
                j] == 1:
            count = 0
            for r in range(i - 1, i + 2):
                for c in range(j - 1, j + 2):
        ...
        for ii in range(i - 1, i + 2):
            ...
            for jj in range(j - 1, j + 2):
                ...
                if num == 1:
                    ...
                    if self.isbv[ii][jj] == True and self.islandseen[ii][
                            jj] == False:
                        if self.status[ii][jj] == 0:
    ...
    def cal_3bv(self):
        ...
        self.num0seen = [[False for j in range(self.column)]
                         for i in range(self.row)]
        self.islandseen = [[False for j in range(self.column)]
                           for i in range(self.row)]
        self.isbv = [[False for j in range(self.column)]
                     for i in range(self.row)]
            ...
            for j in range(self.column):
                ...
                if self.num[i][j] == 0:
                    if self.num0seen[i][j] == True:
                        ...
                        while (self.num0queue.empty() == False):
                            getqueuehead = self.num0queue.get()
                            self.findopis_bfs(getqueuehead[0], getqueuehead[1],
                                              1)
