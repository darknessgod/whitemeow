...
class MineSweeperGUI(superGUI.Ui_MainWindow):
    ...
    def initMineArea(self):
        ...
        self.label.setMinimumSize(
            QtCore.QSize(self.gridsize * self.game.column,
                         self.gridsize * self.game.row))
        ...
        self.label.leftAndRightRelease.connect(
    ...
    def gameStart(self):
        ...
        if self.needtorefresh == True:
            self.initMineArea()
        self.game.pressed = [[0 for j in range(self.game.column)]
                             for i in range(self.game.row)]
        self.game.status = [[0 for j in range(self.game.column)]
                            for i in range(self.game.row)]
        if self.game.gametype != 2:
            self.game.num = [[0 for j in range(self.game.column)]
                             for i in range(self.game.row)]
        self.label.update()
    ...
    def gameFinished(self, result):
        ...
        self.game.intervaltime = max(
            float('%.2f' % (self.game.endtime - self.game.starttime - 0.005)),
            0.01)
        self.timer.stop()
        self.game.cal_3bv()
        if result == 1:
            if self.game.gametype == 1:
                self.game.gamenum += 1
            score = [
                self.game.intervaltime,
                self.game.bbbv / self.game.intervaltime,
                (self.game.intervaltime**1.7) / self.game.bbbv
            ]
    ...
    def gridup(self):
        if self.gridsize <= 46:
            ...
            num, status, pressed = [*self.game.num], [*self.game.status
                                                      ], [*self.game.pressed]
            self.initMineArea()
            self.game.num, self.game.status, self.game.pressed = num, status, pressed
            self.mainWindow.setMaximumSize(self.gridsize * self.game.column,
                                           self.gridsize * self.game.row)
            self.mainWindow.resize(self.mainWindow.minimumSize())
            self.action_gridsize.setText('当前尺寸：%d' % (self.gridsize))
    ...
    def gamerank(self, score):
        ...
        ranks = [1, 1, 1]
        for s in self.datas.stats:
            if s[self.datas.numlevel] == level and s[
                    self.datas.numstyle] == style:
                if score[0] > s[self.datas.numrt]:
                    ranks[0] += 1
                if score[1] < s[self.datas.numbbbv] / s[self.datas.numrt]:
                    ranks[1] += 1
                if score[2] > (s[self.datas.numrt]**
                               1.7) / s[self.datas.numbbbv]:
                    ranks[2] += 1
        return ranks
